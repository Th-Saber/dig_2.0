  ...
  <div class="page_box bb_down">
    ...
    >
      ...
      <el-table-column label="提币地址类型" width="160">
        <template slot-scope="scope">{{ scope.row.addressType }}</template>
      </el-table-column>
      ...
      <el-table-column label="提币时间" width="180" show-overflow-tooltip>
        <template slot-scope="scope">{{
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.state === 2">
            <el-button
              type="primary"
              size="small"
              @click="checkDown(scope.row.withdrawId, true)"
              >审核</el-button
            >
            <el-button
              type="danger"
...
<script>
import { findWithdraw, confirmWithdraw } from "@api/trading";
...
export default {
  ...
  methods: {
    ...
    async searchFn(isFlash) {
      ...
      this.loading = true;
      try {
        let res = await findWithdraw(params);
    ...
    // 验证通过
    checkDown(withdrawId, isOk) {
      this.$confirm(
        `该操作将${isOk ? "通过" : "驳回"}该条提现记录, 是否继续?`,
        "提示",
        {
          ...
          beforeClose: async (action, instance, done) => {
            if (action === "confirm") {
              ...
              try {
                await confirmWithdraw({ withdrawId, isOk });
                this.$message({
                  type: "success",
                  message: "操作成功!",
                });
