<template>
  <div class="user page_box">
    <el-form :model="search" status-icon :rules="rules" ref="ruleForm" inline>
      ...
      <el-form-item label="推荐人" prop="inviterName">
        <el-input
          v-model="search.inviterName"
    ...
    >
      ...
      <el-table-column label="推荐码" align="center" width="120">
        <template slot-scope="scope">
          <el-button
            ...
            size="small"
            plain
            @click="setCodeFn(scope.row)"
          >
            {{ scope.row.markCode }}</el-button
          >
        </template>
      </el-table-column>

      <el-table-column label="是否锁定" width="80" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.state"
            active-color="#ff4949"
            @change="clockUser(scope.row)"
          >
          </el-switch>
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号身份" width="200" align="center">
        <template slot-scope="scope">
          <el-radio-group
...
export default {
  ...
  methods: {
    async searchFn(isFlash) {
      ...
      this.loading = true;
      try {
        let { tel, name, page, size, inviterName, mail } = this.search;
        let params = {
          tel,
          name,
          inviterName,
